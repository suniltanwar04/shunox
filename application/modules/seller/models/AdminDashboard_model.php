...
class AdminDashboard_model extends CI_Model
{
    ...
    public function countUsers(){
			$query = "SELECT Id FROM ". CommonTables::USER." WHERE UserRole > 1 and UserRole != 3";
	...
	public function countOrders(){
			$query = "SELECT Id FROM ". CommonTables::USER_ORDER." WHERE 1=1 and UserId = ".$this->session->userdata('adminId');
			$stmt = $this->pdoConn->prepare($query);
			$stmt->execute();
			$result = $stmt->rowCount();
			return $result;
