                    ...
                    if ($becomedealers) {
                        ...
                        <table id="row-list" class="table table-bordered table-striped custom-table-head">
                            ...
                            foreach ($becomedealers as $becomedealer) {
                                ...
                                <tr>
                                    <td><?php echo $i; ?></td>
                                    <?php if($becomedealer->uniqueId!=''){?>
                                    <td><?php echo $becomedealer->uniqueId; ?></td>
                                    <?php }else{?>
                                        <td></td>
                                    <?php }?>
                                    ...
                                    <td><a href="<?php echo $this->config->item('seller_base_url') . 'view-become-detail-with-dealer-id/'.$becomedealer->id; ?>" class="fa fa-eye-slash" aria-hidden="true"></a>
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    <a class="glyphicon glyphicon-trash" title="Delete" style="cursor: pointer;" id="<?php echo $becomedealer->id?>"></a>
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    <a href="<?php echo $this->config->item('seller_base_url') . 'edit-become-detail-with-dealer-id/'.$becomedealer->id; ?>" style="cursor: pointer;" id="<?php echo $becomedealer->id?>"><i class="fa fa-edit"></i></a>
                                    </td>
                                    
                                    <td> <a href="<?php echo $this->config->item('seller_base_url') . 'view-users/'.$becomedealer->id; ?>">
                                            <i class="fa fa-eye-slash"></i></a></td>
...
$(document).off('click', '.glyphicon').on('click', '.glyphicon', function (e) {
            ...
            redirecturl = '<?php echo base_url()?>seller/become-dealer';
            ...
            bootbox.confirm("Are you sure want to delete this record?", function (result) {
                if (result == true) {
                    ...
                    $.ajax({
                        ...
                        url: $('#baseUrl').val() + "<?php echo CommonConstants::SELLER_URL_SLUG;?>/delete-dealer",
...
$(document).off('click', '.edit').on('click', '.edit', function (e) {
    ...
    if (recordId) {
        ...
        $.ajax({
            ...
            url: $('#baseUrl').val() + "<?php echo CommonConstants::SELLER_URL_SLUG;?>/editDealer",
            success: function (result) {
                $('#loading').css("display", "none");
...
$(document).off('click', '#updateDealer').on('click', '#updateDealer', function (e) {
    ...
    if (!dealerId) {
        ...
        $.ajax({
            ...
            url: $('#base_url').val() + "<?php echo CommonConstants::SELLER_URL_SLUG;?>/updateDealer",
            success: function (result) {
