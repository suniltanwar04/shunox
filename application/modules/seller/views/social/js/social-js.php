...
//                    alert(pageIndex);
        ...
        $(document).off('click', '#saveSocial').on('click', '#saveSocial', function(e){
            e.preventDefault;
            var status1 = false, status2 = false;
            
           
            var addName = $.trim($("#addName").val());
            var addUrl = $.trim($("#url").val());
           ...
           if (!addName){
            ...
            if (!addUrl){
                $("#addUrlError").show().html('Please enter url.');
                $("#url").focus();
            }else {
                status1 = true;
            }
            ...
            if(status1 === true && status2 === true){
                ...
                var formData = new FormData($("#socialAddForm")[0]);
                    $.ajax({
                        type: "POST",
                        data:formData,
                        cache: false,
                        contentType: false,
                        processData: false,
                        url: $('#baseUrl').val() + "<?php echo CommonConstants::SELLER_URL_SLUG;?>/saveSocial",
            ...
            bootbox.confirm("Are you sure to disable this record?", function (result) {
                if (result == true) {
                    ...
                    $.ajax({
                        ...
                        url: $('#baseUrl').val() + "<?php echo CommonConstants::SELLER_URL_SLUG;?>/enableDisableSocial",
                        success: function (result) {
                            $('#loading').css("display", "none");
//                        console.log(result);
//                        return false;
            ...
            bootbox.confirm("Are you sure want to delete this record?", function (result) {
                if (result == true) {
                    ...
                    $.ajax({
                        ...
                        url: $('#baseUrl').val() + "<?php echo CommonConstants::SELLER_URL_SLUG;?>/deleteSocial",
                        success: function (result) {
